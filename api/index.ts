...
app.post(['/fiscal-module/emitir', '/api/fiscal-module/emitir'], authenticate, async (req, res) => {
    ...
    try {
        ...
        if (activeProvider === 'national') {
            ...
            const dpsXml = `<?xml version="1.0" encoding="UTF-8"?><DPS xmlns="http://www.sped.fazenda.gov.br/nfse" versao="1.00"><infDPS Id="${dpsId}"><tpAmb>${inf.tpAmb || tpAmb || 2}</tpAmb><dhEmi>${inf.dhEmi || dhEmi}</dhEmi><verAplic>${verAplic}</verAplic><dCompet>${inf.dCompet || dCompet}</dCompet><tpEmit>1</tpEmit><cLocEmi>${cLocEmi}</cLocEmi><serie>${serieVal}</serie><nDPS>${parseInt(numDpsInt)}</nDPS><prest><CNPJ>${prestCnpjClean}</CNPJ>${prestIM}<regTrib><opSimpNac>${opSimpNac}</opSimpNac><regEspTrib>${regEspTrib}</regEspTrib></regTrib></prest><toma>${tomadorDocXml}<xNome>${inf.toma?.xNome || 'NÃO IDENTIFICADO'}</xNome>${tomadorEndXml}${inf.toma?.email ? `<email>${inf.toma.email}</email>` : ''}</toma><serv>${servLocXml}${servItemXml}</serv>${valoresXml}</infDPS></DPS>`.trim();
