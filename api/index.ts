...
app.post(['/fiscal-module/emitir', '/api/fiscal-module/emitir'], authenticate, async (req, res) => {
    ...
    try {
        ...
        if (activeProvider === 'national') {
            ...
            const ibscbsXml = `<IBSCBS><finNFSe>${finNFSe}</finNFSe><indFinal>${indFinal}</indFinal><cIndOp>${cIndOp}</cIndOp><indDest>${indDest}</indDest><valores><trib><gIBSCBS><CST>${cstVal}</CST><cClassTrib>000001</cClassTrib><gTribRegular><CSTReg>${cstRegVal}</CSTReg><cClassTribReg>000001</cClassTribReg><pAliqEfeRegCBS>${pCbsVal.toFixed(2)}</pAliqEfeRegCBS><vTribRegCBS>${vCbsVal.toFixed(2)}</vTribRegCBS><pAliqEfeRegIBSMun>${pIbsVal.toFixed(2)}</pAliqEfeRegIBSMun><vTribRegIBSMun>${vIbsVal.toFixed(2)}</vTribRegIBSMun></gTribRegular></gIBSCBS></trib></valores></IBSCBS>`;
