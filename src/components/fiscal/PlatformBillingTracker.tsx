...
export function PlatformBillingTracker({ invoices, companySettings, activeProvider }: PlatformBillingTrackerProps) {
    ...
    return (
        <div className="bg-gradient-to-r from-blue-50/30 via-indigo-50/5 to-transparent dark:from-slate-850 dark:via-slate-850/40 dark:to-transparent border border-blue-100/40 dark:border-slate-800/80 rounded-2xl p-3.5 mb-4 animate-in fade-in duration-300 shadow-sm select-none">
            ...
            {/* Expanded Detailed Breakdown Modal */}
            <Modal
                isOpen={isExpanded}
                onClose={() => setIsExpanded(false)}
                title="Demonstrativo Detalhado de Custos"
                subtitle="Visualização detalhada do ciclo de faturamento atual"
                icon={TrendingUp}
                maxWidth="max-w-4xl"
                variant="info"
            >
                <div className="grid gap-5 grid-cols-1 md:grid-cols-2 text-slate-800 dark:text-slate-100">
                    {/* Summary of applied billing */}
                    <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/50 dark:border-slate-800 shadow-sm flex flex-col justify-between">
                        <div>
                            <h5 className="font-bold text-xs uppercase text-gray-400 dark:text-slate-500 tracking-wider mb-4 flex items-center gap-1.5 border-b border-slate-200/50 dark:border-slate-800 pb-2">
                            ...
                            <div className="space-y-4">
                                {providersDetails.map((pd) => {
                                    ...
                                    return (
                                        <div 
                                            key={pd.provider} 
                                            className="p-3.5 rounded-xl bg-white dark:bg-slate-955 border border-slate-150 dark:border-slate-800 shadow-sm"
                                        >
                                            <div className="flex items-center justify-between border-b border-gray-150 dark:border-slate-800/80 pb-2 mb-3">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {tieredEnabled ? (
                                                    <div className="space-y-1.5 py-1 border-t border-gray-100 dark:border-slate-850/50 pt-2 mt-1">
                                                ...
                                                <div className="flex justify-between items-center pt-2 border-t border-gray-150 dark:border-slate-800/80 mt-2">
                                                    <span className="text-[10px] uppercase font-bold text-gray-455 tracking-wider">Subtotal {providerName}</span>
                        ...
                        <div className="mt-4 p-3.5 rounded-xl bg-gradient-to-r from-blue-500/10 via-indigo-500/5 to-transparent border border-blue-500/20 dark:border-blue-900/35 flex justify-between items-center">
                    ...
                    {tieredEnabled ? (
                        <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/50 dark:border-slate-800 shadow-sm flex flex-col justify-between">
                            <div>
                                ...
                                <div className="overflow-hidden rounded-xl border border-gray-155 dark:border-slate-800/80 text-xs bg-white dark:bg-slate-950/40">
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50 dark:bg-slate-900 font-bold text-[10px] text-gray-550 uppercase tracking-wider border-b border-gray-150 dark:border-slate-800">
                                            <tr>
                            ...
                            <div className="mt-4 p-3 bg-blue-50/50 dark:bg-blue-955/15 rounded-xl border border-blue-150/45 text-xs text-blue-800 dark:text-blue-300">
                                <span className="font-bold block mb-0.5">ℹ️ Informação sobre Faixas:</span>
                                O custo por nota diminui automaticamente conforme o volume total acumulado no ciclo atual atinge faixas superiores.
                            </div>
                        </div>
                    ) : (
                        <div className="p-4 rounded-xl bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/50 dark:border-slate-800 shadow-sm flex flex-col justify-center items-center text-center py-10">
                            <span className="text-3xl mb-3">📈</span>
                            <span className="font-extrabold text-sm text-slate-800 dark:text-slate-200">Preço Fixo por Nota</span>
                            <span className="text-xs text-gray-500 mt-1.5 max-w-xs leading-relaxed">
                                Sua conta está configurada com preço fixo por nota emitida. Nenhuma tabela de faixas dinâmica está ativada.
                            </span>
                        </div>
                    )}
                </div>
