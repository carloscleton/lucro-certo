        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="text-center py-10 bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-slate-700">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-4">{t('products.subtitle')}</p>
                    <Button onClick={() => setIsFormOpen(true)}>
                        <Plus size={20} className="mr-2" />
                        {t('common.add')}
                    </Button>
                            ...
                            {products.map((product) => (
                                <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-900 dark:text-gray-100">
                                        {new Intl.NumberFormat(window.__CURRENCY_LOCALE__ || 'pt-BR', { style: 'currency', currency: window.__CURRENCY_CODE__ || 'BRL' }).format(product.price)}
                                    </td>
