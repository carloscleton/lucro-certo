-- Migration to add warranty_module_enabled to companies table and update admin RPC functions
ALTER TABLE public.companies ADD COLUMN IF NOT EXISTS warranty_module_enabled BOOLEAN DEFAULT FALSE;

-- Update comments
COMMENT ON COLUMN public.companies.warranty_module_enabled IS 'Indica se a empresa contratou o módulo de Controle de Garantia e Responsabilidade Técnica';

-- Drop old functions to prevent conflicts when signature changes
DROP FUNCTION IF EXISTS public.get_admin_companies_list();
DROP FUNCTION IF EXISTS public.admin_update_company_config(uuid,boolean,boolean,boolean,boolean,boolean,boolean,text[],jsonb,boolean,numeric,boolean);
...
-- 1. Updated get_admin_companies_list to include warranty_module_enabled
...
-- 2. Updated admin_update_company_config to include warranty_enabled
