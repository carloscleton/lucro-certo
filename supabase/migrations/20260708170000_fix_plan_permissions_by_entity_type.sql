-- Fix function to sync plan permissions respecting entity_type (PF vs PJ)
...
BEGIN
    ...
    IF plan_data IS NOT NULL THEN
        ...
        -- Use as permissões específicas baseado no entity_type da empresa
        IF NEW.entity_type = 'PJ' THEN
            profile_mods := COALESCE(plan_data->'pj_profile_modules', '{}'::jsonb);
            tabs_to_set := COALESCE(plan_data->'pj_settings_tabs', '{}'::jsonb);
        ELSE
            profile_mods := COALESCE(plan_data->'profile_modules', '{}'::jsonb);
            tabs_to_set := COALESCE(plan_data->'settings_tabs', '{}'::jsonb);
        END IF;
        ...
        NEW.settings := jsonb_set(
            ...
            '{settings_tabs}',
            jsonb_build_object(
                'quotes', COALESCE(tabs_to_set->'quotes', '{"admin": false, "member": false}'::jsonb),
                'financial', COALESCE(tabs_to_set->'financial', '{"admin": false, "member": false}'::jsonb),
                'team', COALESCE(tabs_to_set->'team', '{"admin": false, "member": false}'::jsonb),
                'webhooks', COALESCE(tabs_to_set->'webhooks', '{"admin": false, "member": false}'::jsonb),
                'whatsapp', COALESCE(tabs_to_set->'whatsapp', '{"admin": false, "member": false}'::jsonb),
                'payments', COALESCE(tabs_to_set->'payments', '{"admin": false, "member": false}'::jsonb),
                'banking', COALESCE(tabs_to_set->'banking', '{"admin": false, "member": false}'::jsonb),
                'automations', COALESCE(tabs_to_set->'automations', '{"admin": false, "member": false}'::jsonb),
                'fiscal', COALESCE(tabs_to_set->'fiscal', '{"admin": false, "member": false}'::jsonb),
                'subscription', COALESCE(tabs_to_set->'subscription', '{"admin": false, "member": false}'::jsonb),
                'platform', COALESCE(tabs_to_set->'platform', '{"admin": false, "member": false}'::jsonb)
            )
        );
    END IF;
