...
serve(async (req) => {
  try {
    ...
    console.log("Evo Webhook Payload recebido:", JSON.stringify(payload, null, 2))
    ...
    if (!data || data.key.fromMe) {
      return new Response('Mensagem própria ignorada', { status: 200 })
    }
    ...
    const profile = profiles?.find(p => {
      ...
      // Compara os últimos 8 digitos do telefone para evitar problema de DDI/DDD e nono digito
    ...
    // Buscar o post pendente mais recente dessa empresa
    ...
    if (!pendingPosts || pendingPosts.length === 0) {
      console.log('Nenhum post PENDENTE encontrado para a empresa:', profile.company_id)
